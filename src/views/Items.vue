  ...
  <div class="timeline">
    <div class="wrapper">
      <transition-group name="tl-complete" tag="div">
        <div
          v-for="item in items"
          class="timeline-item tl-complete-item"
          ref="timeline"
          :class="{'timeline-bg':item.style}"
          v-bind:key="item.id"
        >
          <div class="timeline-content">
            <div style="float:right;padding: 5px 0;">
              <a style="cursor: pointer;" @click="edit(item.id)">edit</a> |
    ...
    <addDialog @timeline:addOK="added" :visible="addedDialogVisible" :item="selectedItem"></addDialog>
...
import { formatDate, find } from "../common/filter.js";

export default {
  created() {
    this.id = this.$route.params.id;
    this.$axios
      .get("services/app/TimeAxisService/GetItems", {
        params: {
          AxisId: this.id
        }
      })
  ...
  methods: {
    ...
    edit(id) {
      let i = find(this.items, id, x => x.id);
      this.selectedItem = i == -1 ? {} : this.items[i];
      this.$store.commit("setPopStatus", true);
    },
    updateDom() {
      this.$nextTick(() => {
